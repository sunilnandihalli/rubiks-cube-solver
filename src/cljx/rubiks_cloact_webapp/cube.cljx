...
(defn apply-algorithm [rcs ops n]
  (let [rotate (fn rotate [crcs op]
                   ...
                   (map (fn [rubiks-piece]
                          (if-not (= coord (first (rubiks-piece dir))) rubiks-piece
                            (into {} (map (fn [[rp-dir [rp-coord rp-color] :as orig]]
                                          ...
                                          rubiks-piece))))
                        crcs)))]
    (reduce rotate rcs ops)))

(let [faces [:front :back :right :left :top :bottom]]
 (defn face-representation [rcs n]
   (let [n-1 (- n 1)
     ...
     (reduce update-display-state initial-display-state rcs))))

(defn display [rcs n]
  (let [{:keys [front back right left top bottom] :as frcs} (face-representation rcs n)
